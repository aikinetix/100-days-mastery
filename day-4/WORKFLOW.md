# Day 4 — Workflow diagrams (Mermaid)
...
High-level control flow matching `server.js`: router LLM → parse → optional weather fallback → tool (`getWeather` / Open-Meteo) or final LLM answer.
...
flowchart TD
  ...
  Exec -->|yes| GW[getWeather city<br/>Open-Meteo geocode + forecast]
...
sequenceDiagram
  ...
  alt Tool path getWeather
    ...
    Express->>Wx: GET forecast current weather
    Wx-->>Express: temperature, code, wind
...
| **getWeather** | No API key; uses Open-Meteo HTTPS APIs. |
